...
export class StaysAutoApplyService {
  ...
  private static readonly userAllowlistEnv = 'STAYS_AUTO_APPLY_ALLOWED_USER_IDS';
  private static readonly listingAllowlistEnv = 'STAYS_AUTO_APPLY_ALLOWED_LISTING_IDS';
  private static readonly userAllowlistAliasEnv = 'STAYS_AUTO_APPLY_USER_ALLOWLIST';
  private static readonly listingAllowlistAliasEnv = 'STAYS_AUTO_APPLY_LISTING_ALLOWLIST';
  private static readonly cohortEnv = 'STAYS_AUTO_APPLY_COHORT';
  private static readonly requirePricingDecisionEnv = 'STAYS_AUTO_APPLY_REQUIRE_PRICING_DECISION';
  private static readonly requireLiveAllowlistsEnv = 'STAYS_AUTO_APPLY_REQUIRE_LIVE_ALLOWLISTS';
  private static readonly minConfidenceEnv = 'STAYS_AUTO_APPLY_MIN_CONFIDENCE';
  private static readonly minBookingProbabilityEnv = 'STAYS_AUTO_APPLY_MIN_BOOKING_PROBABILITY';
  private static readonly minMultiplierEnv = 'STAYS_AUTO_APPLY_MIN_RECOMMENDED_MULTIPLIER';
  private static readonly maxMultiplierEnv = 'STAYS_AUTO_APPLY_MAX_RECOMMENDED_MULTIPLIER';
  private static readonly blockedRiskFlagsEnv = 'STAYS_AUTO_APPLY_BLOCKED_RISK_FLAGS';
  ...
  async handleCron() {
    ...
    try {
      await this.runCronWithTracking('stays-auto-apply', () => this.processBatch());
    } catch (err) {
  ...
  async processBatch(): Promise<StaysAutoApplyBatchResult> {
    ...
    if (!config.enabled) {
      ...
      return { processed: 0, eligible: 0, applied: 0, dryRun: 0, blocked: 0, errors: 0 };
    }
    ...
    for (const listing of autoListings) {
      ...
      if (allowlist.blockers.length > 0) {
        blocked++;
        this.logBlockedAutoApply(listing, null, allowlist.blockers, allowlist.warnings);
        continue;
      }
      ...
      try {
        ...
        if (!userId) {
          this.logger.warn(`Stays auto-apply listing=${listing.id} sem userId; pulando.`);
          continue;
        }

        const previousPriceCents =
          this.moneyToCents(analise.seuPrecoAtual) ?? this.toIntegerCents(listing.basePriceCents) ?? 0;
        ...
        if (!eligibility.eligible) {
          blocked++;
          this.logBlockedAutoApply(listing, analise, eligibility.blockers, eligibility.warnings, eligibility.audit);
          continue;
        }
  ...
  private getConfig(): StaysAutoApplyConfig {
    const config: StaysAutoApplyConfig = {
      ...
      cohort: process.env[StaysAutoApplyService.cohortEnv]?.trim() || 'event-safe-beta',
      ...
      minConfidence: this.parseConfidenceEnv(StaysAutoApplyService.minConfidenceEnv, 'medium'),
      ...
      blockedRiskFlags: this.parseListEnv(StaysAutoApplyService.blockedRiskFlagsEnv),
    };
    if (config.blockedRiskFlags.size === 0) {
      config.blockedRiskFlags = new Set(StaysAutoApplyService.defaultBlockedRiskFlags);
    }
    return config;
  }
  ...
  private async evaluateEligibility(input: {
    ...
    const { listing, analise, targetDate, previousPriceCents, newPriceCents, config } = input;
    const blockers: string[] = [];
    ...
    const recommendedPriceCents = this.toIntegerCents(decision?.recommendedPriceCents);
    ...
    if (!listing.account?.consentAcceptedAt || !listing.account?.consentVersion) {
      blockers.push('missing_stays_auto_apply_consent');
    }

    if (!audit.rollbackReady) blockers.push('missing_rollback_baseline');
    if (!this.isValidTargetDate(targetDate)) blockers.push('invalid_target_date');

    if (!config.dryRun && config.requireLiveAllowlists) {
      if (config.allowedUserIds.size === 0) blockers.push('missing_user_allowlist_for_live_cohort');
      if (config.allowedListingIds.size === 0) blockers.push('missing_listing_allowlist_for_live_cohort');
    }

    if (!decision) {
      if (config.requirePricingDecision) blockers.push('missing_pricing_decision_snapshot');
    } else {
      if (!this.isSafeDecisionStatus(decision.status)) {
        blockers.push(`unsafe_pricing_decision_status:${decision.status}`);
      }

      if (decision.appliedPriceCents !== null && decision.appliedPriceCents !== undefined) {
        blockers.push('pricing_decision_already_has_applied_price');
      }

      if (this.confidenceRank(decision.confidence) < this.confidenceRank(config.minConfidence)) {
      ...
      if (!decisionIdempotencyKey) warnings.push('missing_pricing_decision_idempotency_key');
      if (decision.guardrails?.cappedRecommendedPrice) warnings.push('recommendation_capped_by_guardrail');
    }

    for (const flag of riskFlags) {
      if (config.blockedRiskFlags.has(flag)) blockers.push(`blocked_risk_flag:${flag}`);
    }
  ...
  private riskFlagsFromDecision(decision?: PricingDecisionSnapshot | null): string[] {
    return this.unique([
  ...
  private confidenceRank(confidence?: EventIntelligenceConfidence | null): number {
  ...
  private multiplierFromPrices(priceCents: number, basePriceCents: number): number | null {
    if (priceCents <= 0 || basePriceCents <= 0) return null;
    return Number((priceCents / basePriceCents).toFixed(2));
  }

  private moneyToCents(valueReais: number | string | null | undefined): number | null {
    const num = typeof valueReais === 'string' ? Number(valueReais) : valueReais;
  ...
  private parseNumberEnv(name: string, fallback: number, min: number, max: number): number {
