...
Escopo: fechar o que depende de acesso humano a Stripe, Brevo, Stays,
Railway e caixas de suporte/privacidade.

## Estado deixado pelo Dev 3

- Dashboard admin mostra go-live Track 3 para Stripe, Brevo, Stays e Suporte/LGPD.
...
2. Brevo: validar remetente/dominio, DKIM/SPF e preencher `BREVO_API_KEY`,
   `EMAIL_SENDER`, `EMAIL_SENDER_NAME` e `RESET_PASS_URL`.
...
7. Executar:
   ...
   - `docs/runbooks/brevo-transactional-smoke.md`
   - `docs/runbooks/stays-beta-private-smoke.md`
   - `docs/runbooks/suporte-lgpd-beta-pago.md`
8. Anexar evidencias de Stripe webhook, envio Brevo, preview/push Stays e inbox suporte/LGPD.
...
- `/admin/dashboard` mostra Stripe, Brevo, Stays e Suporte/LGPD como ready;
- checkout test mode cria assinatura e webhook atualiza banco;
- Brevo entrega ao menos um e-mail real;
