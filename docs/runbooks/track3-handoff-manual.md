...
1. Railway backend: preencher `SUPPORT_EMAIL`, `PRIVACY_EMAIL`,
   `SUPPORT_OWNER_EMAIL`, `PRIVACY_OWNER_EMAIL`.
2. MailerSend: validar dominio, DKIM/SPF e preencher `MAILERSEND_API_KEY`,
   `EMAIL_SENDER`, `FRONT_URL`, `RESET_PASS_URL`.
3. Stripe test mode: preencher `STRIPE_SECRET_KEY`, `STRIPE_PUBLIC_KEY`,
   `STRIPE_WEBHOOK_SECRET` e os 8 `*_PRICE_*`.
...
- preflight strict sai com exit code 0 no ambiente alvo;
