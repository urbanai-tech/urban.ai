...
**Versão:** 1.1 · **Atualizado:** 15/07/2026 · **Status:** política interna em validação; SLA público ainda não aprovado
...
### 1.1 Disponibilidade da API e da aplicação

- **SLO interno atual:** **99,7%** uptime mensal
- **Meta de certificação 10/10:** **99,9%**, somente após janela observada e evidência contínua
- **SLA publicado:** **99.5%** uptime mensal

99.5% = até **3h39 de downtime por mês** permitido. 99.7% = **2h11**.

Medição alvo: monitor externo consulta a home pública e `/health/live` a cada minuto. O endpoint `/health` é readiness autenticado e deve ser monitorado separadamente com `HEALTH_READINESS_TOKEN`; liveness 200 não substitui readiness 200. Downtime = resposta 5xx ou timeout por pelo menos dois pings consecutivos.

**Estado verificado em 15/07/2026:** liveness responde 200; readiness responde 503 porque o token ainda não está configurado no ambiente; o hostname público de status ainda não resolve. Portanto, nenhum SLO/SLA está certificado neste momento.

Probe sintético reproduzível do repositório:

```powershell
npm run slo:probe -- --base-url https://<api-canonica> --path /health/live --requests 100 --concurrency 5 --min-availability 99.9 --max-p95-ms 1000
$env:HEALTH_READINESS_TOKEN='<fornecido-no-shell>'
npm run slo:probe -- --base-url https://<api-canonica> --path /health --requests 20 --concurrency 2
```

O relatório inclui disponibilidade, p50, p95, p99 e categorias de status/erro, sem imprimir corpo de resposta ou valor do token. `npm run slo:probe:self-test` valida localmente o próprio medidor.
...
| E-mail entrega | Painel Brevo | diário |
...
Até existir, o status semanal é compilado manualmente pelo Gustavo a partir das 4 fontes acima e anexado ao relatório quinzenal ao cliente (ver `docs/archive/roadmaps/roadmap-pos-sprint.md` § "Estrutura de Relatórios ao Cliente").
...
(b) eventos de força maior (outages do Railway, Stripe, Google, AWS,
    Brevo, Upstash, Sentry ou backbone de internet);
...
|---|---|---|---|
| 1.1 | 15/07/2026 | Engenharia | Separa liveness/readiness, registra estado real, probe reproduzível, meta 99,9% e Brevo |
