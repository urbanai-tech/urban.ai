...
- [ ] Concluir TLS de `status`, `staging` e `staging-api`. Em 2026-07-21, os três ainda falhavam em TLS estrito; `staging-api` permanecia em `Waiting for DNS update`; o frontend canônico aparecia sem domínio público e com região `us-west2` inválida bloqueando deploys; a status page seguia `built`, mas sem certificado e com `https_enforced=false`. Corrigir a região e reconfirmar a associação existente sem duplicar DNS.
