...
| M2 - Alpha/beta fechado assistido | **83%** | Alpha de contas existentes ganhou quota, preco base, historico sugerido vs. praticado, ROI, painel admin alpha, fallback CSV com gate de qualidade, ocupacao manual admin e E2E read-only admin/anfitriao 100% em producao. | Rodar fluxos mutantes controlados com Gustavo: reprocessar, aceitar/aplicar sugestao, registrar resultado/ocupacao real e acumular 3-5 evidencias reais. |
| M3 - Beta pago / go-live controlado | **65%** | Operacao, release gate, historico de jobs, audit log admin, suporte/LGPD, contatos com contadores globais, ROI, E2E prod 100% read-only, checkout oficial corrigido, smoke Stripe documentado, Billing Portal para upsell/quantity, e-mails billing ligados aos webhooks mockados e migration de `price_updates` reduziram risco; ainda falta prova para cobrar. | Executar Stripe/KYC/smoke, cases, MailerSend real, suporte e legal. |
...
| Admin e operacao | **97% read-only / 72% mutacoes controladas** | Dashboard executivo com eventos, pricing, dataset, billing e Stays; fallback manual; import CSV; `/admin/jobs`; waitlist; alpha ops; ROI; audit logs; contatos publicos; health dos coletores testado; `/admin/stays` mostra bloqueio/liberacao operacional; ocupacao manual em `/admin/quality`; E2E admin prod 100% e smokes mockados de jobs/ocupacao. | Validar mutacoes com fixture real, drill-down por propriedade e alertas externos. |
...
| Testes E2E | **94%** | Spec de prelaunch/waitlist/analytics, F8 waitlist->aceite mockado no endpoint real, smoke publico, contato publico, login/post-login/logout mockado, onboarding import/registro/configuracao/checkout payload mockado, propriedades/preco base/historico/exclusao mockados, jobs admin mockado, ocupacao manual admin mockada, meu plano/quota mockado, readiness de checkout em `/plans`, integracao Stays mockada, dashboard com recomendacao/empty state/aceite/preco aplicado, reset de senha mockado, confirmacao de e-mail mockada, auditor produto admin/anfitriao e E2E autenticado prod 100% para rotas/APIs read-only; builds passaram; deploy prod confirmado. | Ampliar para fluxos mutantes reais: MailerSend real, checkout Stripe test mode em staging e ocupacao com dados observados do beta. |
...
| F4 | 1-2 semanas | Dataset, qualidade e ROI | 68% | Snapshot automatizado/manual, preco aplicado, receita real, propriedades com diaria base/receita/historico mockado, ROI usuario/admin, historico sugerido vs. praticado, features de proximidade e ocupacao manual admin avancaram; MAPE/cases ainda dependem de dados reais | Snapshots por 7 dias, ocupacao real capturada, preco aplicado capturado, relatorio de qualidade inicial. |
| F5 | 3-5 dias | Billing e planos | 78% | Checkout ganhou validacao de DTO/quantity, `/plans` usa `loadStripe` oficial e tem smoke de readiness para preco ausente/chave Stripe ausente/plano sob consulta, onboarding cobre payload anual/quantity, Billing Portal cria sessao autenticada para gerenciar plano/quantity, templates de assinatura/cancelamento/falha/quota tem teste unitario, webhooks de assinatura ativa/cancelamento/falha e cadastro por quota chamam MailerSend best-effort, `/my-plan` mostra assinatura/ciclo/quota e nao quebra com assinatura alpha, sync check admin mostra gate 8/8/sem chave, docs/env apontam para runbooks corretos e testes/builds cobrem billing; falta executar KYC/test-live e ponta-a-ponta | Checkout/webhook/quota/cancelamento/portal testados; status de pagamentos saneado. |
...
| F7 | 1-2 semanas em paralelo | Admin, ops e confiabilidade | 97% read-only / 72% mutacoes controladas | Dashboard executivo + jobs + historico + alpha ops + ROI + audit logs + contatos publicos + collectors health testado + Stays readiness + ocupacao manual admin + E2E admin prod 100% + jobs/ocupacao admin mockados; staging real/alertas externos e mutacoes reais pendentes | Jobs/admin/audit/staging/observabilidade cobrindo operacao diaria. |
