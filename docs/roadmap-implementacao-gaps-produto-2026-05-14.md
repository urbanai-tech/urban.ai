...
| Eventos e coletores | **64%** | Runner estruturado, cron configuravel, Docker corrigido, fontes gratuitas/camadas ja codadas, health alinhado com aliases/criticidade do runner, fallback manual destacado no admin e import CSV com meta de 100 eventos. | Medir eventos futuros reais apos deploy e popular 100-200 eventos SP/30d. |
...
| Admin e operacao | **71%** | Dashboard executivo com eventos, pricing, dataset, billing e Stays; alerta de fallback manual quando eventos futuros ficam abaixo do gate; import CSV mostra meta/qualidade do lote; `/admin/jobs` roda geocoder/reset/snapshot com historico persistente e permite abrir resultado antigo; waitlist permite reenvio/copia de convite. | Audit log amplo, drill-down por propriedade, alertas externos. |
...
| F2 | 3-7 dias | Eventos vivos | 64% | Runner/cron/container corrigidos, health menos ruidoso, fallback manual guiado no admin e import CSV com meta de lote; falta medir base real apos deploy | >= 200 eventos futuros SP/30d ou fallback manual ativo; coletores com lastSeen < 48h. |
