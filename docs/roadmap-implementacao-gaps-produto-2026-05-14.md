...
| F9 | apos gates | Beta pago / go-live controlado | 37% | Nao liberado; release gate, historico, staging drill, suporte/LGPD, smoke autenticado, admin e lifecycle billing por webhook estao mais claros, mas Stripe real/cases seguem bloqueando | 3 cases, billing ok, suporte/termos/observabilidade ok. |
...
| `workspace atual` - `feat: send billing lifecycle emails` | F, J | `PaymentsService` importa `MailerModule` e chama e-mails HTML best-effort nos webhooks `checkout.session.completed`, `customer.subscription.deleted` e `invoice.payment_failed`; Jest valida envio mockado e status sem depender de MailerSend real. |
...
| F9 | P1 | E-mails transacionais: assinatura ativa, falha, cancelada, quota warning/exceeded | Mailersend | Templates e disparos mockados de assinatura/cancelamento/falha testados; falta MailerSend real e quota warning/exceeded nos eventos certos. |
