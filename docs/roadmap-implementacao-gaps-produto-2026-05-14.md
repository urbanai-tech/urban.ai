...
| M2 - Beta fechado assistido | **55%** | O core ganhou mais operacao: preco aplicado alimenta dataset, admin roda snapshot, coletores alertam com menos falso positivo e os smokes de recomendacao/dataset ficaram formalizados. | Eventos futuros suficientes, recomendacoes com data atual, snapshots por 7 dias e amostra real de preco aplicado. |
...
| Infra/release | **60%** | Health/config melhores, Docker frontend/webscraping corrigido, runner evita sucesso falso, runbook de release gate, runbook de fallback de eventos, smokes de recomendacao/dataset, protocolo para devs/agentes e migration de historico admin. | Staging, branch protection, smoke automatizado, restore drill. |
