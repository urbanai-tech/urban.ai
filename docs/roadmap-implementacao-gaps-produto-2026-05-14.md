...
| M2 - Alpha/beta fechado assistido | **70%** | Alpha de contas existentes ganhou quota, preco base, historico sugerido vs. praticado, ROI, painel admin alpha e smoke autenticado condicional para operar recomendacoes reais. | Rodar E2E autenticado com Gustavo/admin, reprocessar apos coletores e acumular 3-5 resultados reais. |
| M3 - Beta pago / go-live controlado | **57%** | Operacao, release gate, historico de jobs, audit log admin, suporte/LGPD, contatos, ROI, smoke autenticado admin/ROI, validação de checkout, smoke Stripe documentado e migration de `price_updates` reduziram risco; ainda falta prova para cobrar. | Executar Stripe/KYC/smoke, cases, billing real, suporte e legal. |
| M4 - Go-live publico | **38%** | Base operacional mais madura, caminho de cases, admin mais completo, cobertura E2E maior e Stays mais seguro por fail-closed; prova de mercado ainda insuficiente. | 3 cases auditados, cobertura alta, dataset/ROI, suporte, observabilidade e billing maduros. |
...
| Eventos e coletores | **67%** | Runner estruturado, cron configuravel, Docker corrigido, fontes gratuitas/camadas ja codadas, health alinhado com aliases/criticidade do runner, UI admin mostra status/critical/env faltante/sinais, fallback manual destacado no admin, import CSV com meta de 100 eventos e runbook operacional. | Medir eventos futuros reais apos deploy e popular 100-200 eventos SP/30d. |
| Recomendacoes de preco | **65%** | Job com contadores, dedupe, motivos de falha, `criadoEm` atualizado, dashboard, filtros de qualidade de evento, feedback de resultado, reprocessamento alpha e smoke autenticado condicional de dashboard/admin/alpha. | Executar o smoke autenticado com Gustavo e atingir >=70% de cobertura beta. |
| Dataset, IA e ROI | **63%** | Diagnostics, snapshots usando preco armazenado, UI de preco aplicado, preco aplicado em `PriceSnapshot`, ROI usuario/admin, historico sugerido vs. real e smoke autenticado condicional para `/admin/roi`. | Executar smoke com beta tester, ocupacao, features diarias, 7 dias de snapshots, MAPE/cases. |
| Billing e Stripe | **60%** | Resolver de Price IDs, seed idempotente, erro claro, DTO/serviço validam plano/ciclo/quantity antes do checkout, webhook/status, migration `peding` e runbook de smoke ponta-a-ponta. | Executar smoke checkout/webhook/quota/cancelamento e KYC/live. |
| Stays automatico | **34%** | Produto reposicionado como beta privado; `price_updates` tem migration; conector/servico agora tendem a falhar fechado sem `STAYS_API_BASE_URL` e `STAYS_TOKEN_ENCRYPTION_KEY`; runbook cobre credencial/criptografia/push/rollback. | Credenciais reais/sandbox, executar connect/sync/push/rollback e consentimento auditavel. |
| Admin e operacao | **86%** | Dashboard executivo com eventos, pricing, dataset, billing e Stays; fallback manual; import CSV; `/admin/jobs`; waitlist; alpha ops; ROI; audit logs; contatos publicos; health dos coletores com status/criticidade/envs/aliases/stale/missing_key e testes. | Validar paineis com conta admin real, drill-down por propriedade e alertas externos. |
| Infra/release | **69%** | Health/config melhores, Docker frontend/webscraping corrigido, runner evita sucesso falso, runbook de release gate, staging release drill, runbook de fallback de eventos, smokes de recomendacao/dataset/billing/Stays, smoke Playwright publico, smoke autenticado condicional, protocolo para devs/agentes, migration de historico admin e migration de `price_updates`. | Executar staging real, branch protection, ampliar smoke automatizado e restore drill. |
...
| Marketing/beta | **43%** | Atribuicao/referral, contato publico persistido, aceite waitlist testado no endpoint atual e runbook de beta assistido definem ICP, onboarding, relatorio semanal e template de case. | Recrutar 20-30 leads, ativar 5-10 anfitrioes e produzir cases reais. |
| Testes E2E | **56%** | Spec de prelaunch/waitlist/analytics, F8 waitlist->aceite mockado no endpoint real, smoke Playwright publico e smoke autenticado condicional para dashboard/admin/alpha/ROI; builds passaram; deploy prod confirmado. | Rodar com credenciais reais e ampliar login, onboarding, dashboard com recomendacao, reset/e-mail e checkout. |
...
| F2 | 3-7 dias | Eventos vivos | 70% | Runner/cron/container corrigidos, keys cadastradas no webscraping/backend, health admin mostra status/criticidade/env faltante, fallback manual guiado e runbook operacional; falta medir eventos reais apos execucao | >= 200 eventos futuros SP/30d ou fallback manual ativo; coletores com lastSeen < 48h. |
| F3 | 3-7 dias | Recomendacoes novas | 65% | Reprocessamento alpha, filtros de qualidade de eventos, preco base, feedback de resultado e smoke autenticado condicional foram implementados; falta rodar com Gustavo | Recomendacoes com `criado_em` atual; cobertura >= 70% dos imoveis ativos em regioes cobertas. |
| F4 | 1-2 semanas | Dataset, qualidade e ROI | 63% | Snapshot automatizado/manual, preco aplicado, receita real, ROI usuario/admin, historico sugerido vs. praticado e smoke `/admin/roi` avancaram; ocupacao/features/MAPE ainda pendentes | Snapshots por 7 dias, preco aplicado capturado, relatorio de qualidade inicial. |
| F5 | 3-5 dias | Billing e planos | 60% | Checkout ganhou validação de DTO/quantity e testes mockados; runbook de smoke Stripe definido, falta executar KYC/test-live e ponta-a-ponta | Checkout/webhook/quota/cancelamento testados; status de pagamentos saneado. |
| F6 | 1-2 semanas apos credenciais | Stays beta privado | 34% | Mantido como beta privado/oculto; `price_updates` tem migration e a integracao passou a falhar fechado sem envs; runbook de credencial/criptografia/push/rollback definido, execucao pendente | Connect/sync/push/rollback testado com conta real ou sandbox. |
| F7 | 1-2 semanas em paralelo | Admin, ops e confiabilidade | 86% | Dashboard executivo + jobs + historico + alpha ops + ROI + audit logs + contatos publicos + collectors health mais acionavel/testado + smoke publico/autenticado; staging real/alertas externos pendentes | Jobs/admin/audit/staging/observabilidade cobrindo operacao diaria. |
| F8 | 2 semanas | Beta fechado assistido | 48% | Alpha assistido com Gustavo pode rodar; waitlist, aceite mockado no endpoint real e playbook estao prontos para ampliar para 5-10 anfitrioes | 5-10 anfitrioes ativos, feedback semanal, primeiros cases. |
| F9 | apos gates | Beta pago / go-live controlado | 35% | Nao liberado; release gate, historico, staging drill, suporte/LGPD, smoke autenticado e admin estao mais claros, mas billing/cases seguem bloqueando | 3 cases, billing ok, suporte/termos/observabilidade ok. |
