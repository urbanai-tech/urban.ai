...
| M3 - Beta pago / go-live controlado | **41%** | Operacao, release gate, historico de jobs e smoke Stripe documentado melhoraram, mas ainda nao ha prova suficiente para cobrar com tranquilidade. | Executar Stripe/KYC/smoke, cases, audit log amplo, suporte e legal. |
...
| Infra/release | **61%** | Health/config melhores, Docker frontend/webscraping corrigido, runner evita sucesso falso, runbook de release gate, runbook de fallback de eventos, smokes de recomendacao/dataset/billing, protocolo para devs/agentes e migration de historico admin. | Staging, branch protection, smoke automatizado, restore drill. |
