...
| M3 - Beta pago / go-live controlado | **55%** | Operacao, release gate, historico de jobs, audit log admin, suporte/LGPD, contatos, ROI, smoke autenticado admin/ROI e smoke Stripe documentado reduziram risco; ainda falta prova para cobrar. | Executar Stripe/KYC/smoke, cases, billing real, suporte e legal. |
| M4 - Go-live publico | **37%** | Base operacional mais madura, caminho de cases, admin mais completo e cobertura E2E maior; prova de mercado ainda insuficiente. | 3 cases auditados, cobertura alta, dataset/ROI, suporte, observabilidade e billing maduros. |
...
| Admin e operacao | **84%** | Dashboard executivo com eventos, pricing, dataset, billing e Stays; fallback manual; import CSV; `/admin/jobs`; waitlist; alpha ops; ROI; audit logs; contatos publicos; health dos coletores com status/criticidade/envs e smoke autenticado admin. | Validar paineis com conta admin real, drill-down por propriedade e alertas externos. |
| Infra/release | **68%** | Health/config melhores, Docker frontend/webscraping corrigido, runner evita sucesso falso, runbook de release gate, staging release drill, runbook de fallback de eventos, smokes de recomendacao/dataset/billing/Stays, smoke Playwright publico, smoke autenticado condicional, protocolo para devs/agentes e migration de historico admin. | Executar staging real, branch protection, ampliar smoke automatizado e restore drill. |
...
| F7 | 1-2 semanas em paralelo | Admin, ops e confiabilidade | 85% | Dashboard executivo + jobs + historico + alpha ops + ROI + audit logs + contatos publicos + collectors health mais acionavel + smoke publico/autenticado; staging real/alertas externos pendentes | Jobs/admin/audit/staging/observabilidade cobrindo operacao diaria. |
