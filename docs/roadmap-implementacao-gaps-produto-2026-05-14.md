...
| Testes E2E | **84%** | Spec de prelaunch/waitlist/analytics, F8 waitlist->aceite mockado no endpoint real, smoke publico, contato publico, jobs admin mockado, meu plano/quota mockado, integracao Stays mockada, dashboard com recomendacao/empty state, reset de senha mockado, auditor produto admin/anfitriao e E2E autenticado prod 100% para rotas/APIs read-only; builds passaram; deploy prod confirmado. | Ampliar para fluxos mutantes: onboarding completo, recomendacao aplicada, confirmacao de e-mail real e checkout. |
