...
| M2 - Alpha/beta fechado assistido | **80%** | Alpha de contas existentes ganhou quota, preco base, historico sugerido vs. praticado, ROI, painel admin alpha e E2E read-only admin/anfitriao 100% em producao. | Rodar fluxos mutantes controlados com Gustavo: reprocessar, aceitar/aplicar sugestao, registrar resultado e acumular 3-5 evidencias reais. |
| M3 - Beta pago / go-live controlado | **63%** | Operacao, release gate, historico de jobs, audit log admin, suporte/LGPD, contatos com contadores globais, ROI, E2E prod 100% read-only, checkout oficial corrigido, smoke Stripe documentado e migration de `price_updates` reduziram risco; ainda falta prova para cobrar. | Executar Stripe/KYC/smoke, cases, billing real, suporte e legal. |
...
| Billing e Stripe | **68%** | Resolver de Price IDs, seed idempotente, erro claro, DTO/servico validam plano/ciclo/quantity antes do checkout, `/plans` usa Stripe.js oficial sem fallback inexistente, `/my-plan` esta defensivo para alpha/payload sem `plan.amount`, webhook/status, migration `peding` e runbook de smoke ponta-a-ponta. | Executar smoke checkout/webhook/quota/cancelamento e KYC/live. |
...
| Marketing/beta | **45%** | Atribuicao/referral, contato publico persistido com triagem admin por status global, aceite waitlist testado no endpoint atual e runbook de beta assistido definem ICP, onboarding, relatorio semanal e template de case. | Recrutar 20-30 leads, ativar 5-10 anfitrioes e produzir cases reais. |
| Testes E2E | **78%** | Spec de prelaunch/waitlist/analytics, F8 waitlist->aceite mockado no endpoint real, smoke publico, auditor produto admin/anfitriao e E2E autenticado prod 100% para rotas/APIs read-only; builds passaram; deploy prod confirmado. | Ampliar para fluxos mutantes: onboarding completo, recomendacao aplicada, reset/e-mail e checkout. |
...
| F3 | 3-7 dias | Recomendacoes novas | 70% | Reprocessamento alpha, filtros de qualidade de eventos, preco base, feedback de resultado e E2E host read-only 100% foram implementados; falta rodar mutacao com Gustavo | Recomendacoes com `criado_em` atual; cobertura >= 70% dos imoveis ativos em regioes cobertas. |
...
| F5 | 3-5 dias | Billing e planos | 68% | Checkout ganhou validacao de DTO/quantity, `/plans` usa `loadStripe` oficial, `/my-plan` nao quebra com assinatura alpha e testes/builds cobrem billing; runbook de smoke Stripe definido, falta executar KYC/test-live e ponta-a-ponta | Checkout/webhook/quota/cancelamento testados; status de pagamentos saneado. |
...
| F7 | 1-2 semanas em paralelo | Admin, ops e confiabilidade | 95% read-only | Dashboard executivo + jobs + historico + alpha ops + ROI + audit logs + contatos publicos + collectors health testado + Stays readiness + E2E admin prod 100%; staging real/alertas externos e mutacoes pendentes | Jobs/admin/audit/staging/observabilidade cobrindo operacao diaria. |
| F8 | 2 semanas | Beta fechado assistido | 50% | Alpha assistido com Gustavo pode rodar; waitlist, aceite mockado no endpoint real, contatos com triagem por status global e playbook estao prontos para ampliar para 5-10 anfitrioes | 5-10 anfitrioes ativos, feedback semanal, primeiros cases. |
