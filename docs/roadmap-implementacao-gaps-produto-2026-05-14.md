...
| Recomendacoes de preco | **66%** | Job com contadores, dedupe, motivos de falha, `criadoEm` atualizado, dashboard, filtros de qualidade de evento, feedback de resultado, reprocessamento alpha, smoke mockado de recomendacao/empty state e smoke autenticado condicional de dashboard/admin/alpha. | Executar o smoke autenticado com Gustavo e atingir >=70% de cobertura beta. |
...
| Testes E2E | **87%** | Spec de prelaunch/waitlist/analytics, F8 waitlist->aceite mockado no endpoint real, smoke publico, contato publico, onboarding import Airbnb mockado, jobs admin mockado, meu plano/quota mockado, readiness de checkout em `/plans`, integracao Stays mockada, dashboard com recomendacao/empty state, reset de senha mockado, confirmacao de e-mail mockada, auditor produto admin/anfitriao e E2E autenticado prod 100% para rotas/APIs read-only; builds passaram; deploy prod confirmado. | Ampliar para fluxos mutantes: onboarding registro/configuracao/pagamento, recomendacao aplicada, MailerSend real e checkout Stripe test mode. |
...
| F3 | 3-7 dias | Recomendacoes novas | 71% | Reprocessamento alpha, filtros de qualidade de eventos, preco base, feedback de resultado, smoke mockado de recomendacao/empty state e E2E host read-only 100% foram implementados; falta rodar mutacao com Gustavo | Recomendacoes com `criado_em` atual; cobertura >= 70% dos imoveis ativos em regioes cobertas. |
...
| M - Testes frontend | Specs E2E de prelaunch/waitlist/analytics, F8 waitlist->aceite no endpoint real, smoke publico de release, contato publico, onboarding import Airbnb mockado, jobs admin mockado, meu plano/quota, readiness de checkout em `/plans`, integracao Stays, dashboard com recomendacao/empty state, reset de senha mockado, confirmacao de e-mail mockada e smoke autenticado condicional foram adicionadas. Builds locais passaram nas rodadas anteriores e deploy prod do ultimo pacote ficou verde. | Rodar Playwright autenticado em prod/staging com Gustavo/admin e confirmar GA/Pixel com consentimento. | Smokes completos: onboarding registro/configuracao/pagamento, dashboard com recomendacao aplicada, MailerSend real e checkout Stripe test mode. |
...
| ID | Prioridade | Tarefa | Dependencia | Criterio de aceite |
|---|---|---|---|---|
| M1 | P0 | Smoke Playwright: login/logout, waitlist, onboarding, dashboard, propriedades, plans | Staging | Onboarding import mockado coberto; falta login/logout/propriedades em staging real. |
...
| M3 | P1 | Smoke checkout com Stripe test mode | F2/F4 | Readiness mockado coberto; falta checkout/retorno com Stripe test mode. |
