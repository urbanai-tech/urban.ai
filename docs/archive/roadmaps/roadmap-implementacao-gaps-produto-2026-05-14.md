...
Base: `docs/archive/audits/auditoria-produto-lancamento-2026-05-13.md`, `docs/archive/roadmaps/roadmap-pos-sprint.md`, `docs/archive/plans/backlog-produto-pos-assuncao.md`, `docs/runbooks/go-live-manual-checklist.md`, commits recentes em `main/origin` e codigo em `Urban-front-main/`, `urban-ai-backend-main/` e `urban-webscraping-main/`.
...
- **Complemento multiagentes em andamento:** o commit `deba730` trouxe `OnboardingDripService` e backfill geocoder; a rodada seguinte adicionou o backend real de `/admin/properties`, `/admin/properties/:id` e `/admin/properties/:id/reprocess`, alem de corrigir riscos encontrados no drip/backfill. Typecheck backend passou em 2026-05-17.
- **Limites encontrados:** o drill-down admin agora tem contrato backend, mas ainda precisa smoke autenticado contra dados reais e deploy Railway do commit novo; o host dashboard mobile ficou visualmente sem overflow, mas a auditoria com mocks caiu em paywall/cookies, entao ainda precisa de smoke autenticado real sem overlays.
...
| M2 - Alpha/beta fechado assistido | **88%** | Alpha de contas existentes esta operavel; UI do anfitriao ganhou RecommendationCard, shell premium e menos ruido visual; admin ganhou drill-down visual + backend real de imoveis. | Smoke autenticado do novo drill-down/reprocess, fluxos mutantes controlados com Gustavo, registro de resultado/ocupacao real e 3-5 evidencias reais. |
| M3 - Beta pago / go-live controlado | **67%** | Billing portal, checkout oficial, admin mais completo, drip D1/D3/D7 e release docs reduzem risco; design ficou pronto para cobrar, mas a prova de valor ainda nao. | Stripe/KYC/smoke ponta-a-ponta, MailerSend real, suporte/legal, cases e monitoramento de incidentes. |
| M4 - Go-live publico | **44%** | Base de produto, design, onboarding e operacao evoluiu bastante, mas ainda falta mercado, dados reais, suporte e observabilidade madura. | 3 cases auditados, cobertura alta de eventos/recomendacoes, dataset/ROI real, billing e suporte prontos. |
...
| Eventos e coletores | **73%** | Runner/cron/health/fallback manual/import CSV/geocoder readiness estao instrumentados; backfill geocoder esta mais seguro para dry-run e relacoes TypeORM. | Ativar Geocoding API Google, rodar backfill dos imoveis pendentes e popular 100-200 eventos SP/30d. |
| Recomendacoes de preco | **72%** | Job, filtros, feedback, aceite/aplicacao, RecommendationCard e reprocess admin por imovel melhoraram o centro de valor do produto. | Smoke autenticado com Gustavo e cobertura >=70% dos imoveis beta com recomendacao ou motivo claro. |
...
| Admin e operacao | **100% read-only / 87% mutacoes controladas** | AdminShell, dashboards, jobs, waitlist, alpha ops, ROI, audit logs, contatos, Stays readiness, ocupacao manual, drill-down visual e backend `/admin/properties` estao em codigo. | Smoke autenticado com fixture real, alertas externos e comparacao de KPIs com banco. |
| Infra/release | **74%** | Health/config/runbooks/smokes existem; Railway prod esta verde no pacote anterior; typecheck backend passou apos os endpoints admin-properties. | Deploy Railway do commit novo, staging real, branch protection, restore drill e smoke pos-deploy autenticado. |
...
| Testes E2E e QA | **94% mock/read-only / 57% mutante real** | Suite Playwright cobre muitos fluxos mockados; E2E prod read-only anterior foi 100%; typecheck backend passou apos drip/backfill/admin-properties. | Smokes mutantes reais: MailerSend, Stripe test mode, ocupacao observada, recomendacao/aplicacao com Gustavo e deploy do commit novo. |
| Design system e UX premium | **90%** | Publico/admin/anfitriao agora compartilham marca, tokens e componentes; login, shells e erro pages estao muito acima do baseline anterior e ja foram publicados no Front Railway. | WCAG AA, remover overlays da auditoria, validar mobile autenticado real e formalizar `docs/product/DESIGN-SYSTEM.md`. |
...
| F7 | 1-2 semanas em paralelo | Admin, ops e confiabilidade | 100% read-only / 87% mutacoes controladas | AdminShell, jobs, audit, contatos, ROI, Stays readiness, ocupacao manual e `/admin/properties` visual + backend estao prontos; mutacoes reais ainda precisam smoke autenticado. | Jobs/admin/audit/staging/observabilidade cobrindo operacao diaria. |
...
| F9 | apos gates | Beta pago / go-live controlado | 42% | Visual, billing e admin estao bem melhores, mas Stripe real/cases/suporte/legal seguem bloqueando cobranca ampla. | 3 cases, billing ok, suporte/termos/observabilidade ok. |
...
| C - Eventos e coletores | `run_all_collectors.sh` agora chama runner Python estruturado; coletores opcionais sem chave viram `skipped/missing_key`; falha critica retorna exit code; resumo machine-readable foi adicionado. Cron ficou configuravel (`COLLECTOR_CRON_INTERVAL_SECONDS`, `RUN_COLLECTORS_ON_BOOT`), roda modulo Python direto no container e o spider legado correto e `even3`. Health admin respeita aliases/criticidade real e a UI mostra status, criticidade, aliases, variaveis ausentes, `stale`, `missing_key`, `no_events` e sinais operacionais; ha testes cobrindo alias/missing env/stale. Dashboard admin mostra fallback manual quando eventos futuros ficam abaixo do gate beta; import CSV agora normaliza `sourceLabel` e barra data passada/UF fora de SP antes do ingest. Geocoder expoe readiness, ultimo run e falhas acionaveis no status/admin jobs. API Football, SerpAPI, Tavily e Firecrawl foram cadastradas nos ambientes de webscraping/backend conforme autorizado. | Rodar `DRY_RUN` e execucao real no container/Railway; confirmar `lastSeen`, `created24h`, eventos futuros no banco e se cada key esta sendo lida no servico correto; apos ativar Geocoding API, rodar geocoder e confirmar queda dos pendentes. | Popular 100-200 eventos futuros SP; revisar termos/fallback por fonte e remover fontes quebradas ou ruidosas. |
...
| E - Dataset e ROI | `DatasetCollectorService` ganhou diagnostics, snapshot usando preco armazenado, preco aplicado em `PriceSnapshot`, diaria base inicial, receita real mensal, propriedades com smoke mockado de diaria/receita/historico, historico sugerido vs. praticado, features de proximidade, `/my-roi`, `/admin/roi`, endpoint/lista admin de ocupacao e smoke E2E mockado de ocupacao manual. | Confirmar deploy/migrations em prod, rodar snapshot automatico por 7 dias, validar ROI do usuario Gustavo e comparar com dados reais de reserva/ocupacao. | Alimentar ocupacao com observacoes reais ou import futuro, MAPE/cases e fonte externa AirROI/GCP quando aprovada. |
| F - Billing e Stripe | Resolvedor unico de Price ID por plano/ciclo; seed de planos ficou idempotente; checkout falha com erro claro se Stripe/URLs nao estiverem configurados; DTO valida plano/ciclo/quantity e o service rejeita quantity nao numerico antes de chamar Stripe; `/plans` usa `loadStripe`, remove fallback para rota inexistente e tem smoke de readiness para preco ausente/chave Stripe ausente/plano sob consulta; onboarding cobre payload anual/quantity; `/payments/billing-portal-session` cria sessao autenticada do Stripe Billing Portal para upsell/quantity; `/my-plan` agora expoe CTA de gerenciamento que abre o Billing Portal para assinaturas Stripe reais e oculta a acao em alpha assistido; templates transacionais de assinatura/cancelamento/falha/quota tem teste unitario; webhooks de checkout, subscription deleted e invoice failed disparam e-mails best-effort; quota warning/exceeded dispara no fluxo de cadastro quando cruza 80% ou bloqueia N+1; `/my-plan` mostra ciclo, quota e disponibilidade; admin sync check separa Price IDs faltantes/sem chave e mostra gate 8/8; docs/env/README apontam para runbooks reais; webhook normaliza status; migration corrige `peding` para `pending`. Runbook `docs/runbooks/stripe-billing-smoke.md` define prova de sync check, checkout, webhook, quota e cancelamento. | Executar smoke Stripe: checkout, webhook, quota, cancelamento, portal real, sync check 8/8 e disparos MailerSend reais. Confirmar KYC/live mode. | Validacao real do Billing Portal e envio MailerSend em staging/prod. |
...
| M - Testes frontend | Specs E2E de prelaunch/waitlist/analytics, F8 waitlist->aceite no endpoint real, smoke publico de release, contato publico, login/post-login/logout mockado, onboarding import/registro/configuracao/checkout payload mockado, propriedades/preco base/historico/exclusao mockados, jobs admin mockado, meu plano/quota/Billing Portal mockado, readiness de checkout em `/plans`, integracao Stays, dashboard com recomendacao/empty state/aceite/preco aplicado, reset de senha mockado, confirmacao de e-mail mockada e smoke autenticado condicional foram adicionadas. `tsc --noEmit` e `e2e/my-plan-billing.spec.ts` passaram em 2026-05-16 para a conexao do Billing Portal; `tsc --noEmit` front/backend e Jest focado do geocoder passaram em 2026-05-16 para readiness/ultimo run. Builds locais passaram nas rodadas anteriores e deploy prod do ultimo pacote ficou verde. | Rodar Playwright autenticado em prod/staging com Gustavo/admin e confirmar GA/Pixel com consentimento. | Smokes completos: MailerSend real e checkout Stripe test mode em staging. |
...
| `deba730` + workspace atual - drip/backfill/admin-properties | C, D, H, I, M | Onboarding D1/D3/D7 ganhou service/cron e endpoint admin manual; backfill geocoder dos imoveis "A definir" ganhou script e runbook. Auditoria multiagentes corrigiu riscos no drip/backfill e criou `AdminPropertiesModule` isolado com `GET /admin/properties`, `GET /admin/properties/:id` e `POST /admin/properties/:id/reprocess`, fechando o contrato esperado pelo front sem mexer no `AdminController` em conflito. |

**Score apos esta rodada (placar 2.0):**
- M1 Pre-lancamento seguro: **93%** (login premium + paginas erro + continuidade visual publicados)
- M2 Beta fechado: **88%** (RecommendationCard + shell host + drill-down admin visual/backend em codigo)
- M3 Beta pago: **67%** (billing/admin/e-mails ajudam, mas ainda sem prova real para cobrar)
- M4 Go-live publico: **44%** (melhor design, onboarding e operacao, ainda sem cases/dados suficientes)
...
- Pendente no snapshot: Storybook (P3, nao bloqueante), documento `docs/product/DESIGN-SYSTEM.md` formal, audit WCAG AA com checklist preenchido e smoke autenticado sem overlays de cookie/paywall.
...
| Visual admin properties | AdminShell e fallback funcionam; backend real agora esta em codigo no workspace | Falta deploy Railway e smoke autenticado real para tirar o risco operacional. |
| Paginas 403/404 | Visual premium consistente | Texto enorme funciona como marca, mas deve entrar em audit WCAG/mobile para garantir leitura em telas menores. |

**Decisao de produto:** o design agora esta perto do padrao das paginas publicas, mas a prontidao comercial continua limitada por validacao real: recomendacao aplicada, Stripe real/test mode, e-mail real, leads reais e dados de ocupacao/ROI.
...
| `workspace atual` - `feat: expose geocoder readiness in admin jobs` | C, I, M | `/events/geocoder/status` passa a retornar readiness, estado de execucao e ultimo run em memoria; `/admin/jobs` mostra configuracao pendente ou primeira falha do ultimo run, bloqueando execucao sem `GOOGLE_MAPS_API_KEY` e deixando `REQUEST_DENIED` visivel sem abrir logs. Jest cobre status, falha e sucesso do geocoder. |
...
| `workspace atual` - `feat: add admin manual occupancy flow` | E, I, M | `/admin/occupancy/properties` lista imoveis elegiveis sem expor dados sensiveis; `/admin/quality` permite registrar status, diaria anunciada e receita real por dia usando `POST /admin/occupancy/manual`; Jest cobre o contrato do backend e Playwright cobre o fluxo mockado com payload em centavos. |
...
| `workspace atual` - `feat: expose billing portal session` | F, M | Backend adiciona `POST /payments/billing-portal-session`, resolve customer Stripe por `Payment.customerId` ou e-mail e cria sessao do Billing Portal com return URL configuravel; Jest cobre sucesso, fallback e Stripe ausente. |
...
| A5 | P1 | Atualizar `docs/product/business/base-socios.md` com estado honesto pos-auditoria | Este roadmap | Socios veem diferenca entre fundacao pronta e produto validado. |
...
| E5 | P0 | Criar fluxo de captura de ocupacao minimo manual | Produto/UX | **Implementado no admin**: operador marca booked/available/blocked/unknown, diaria anunciada e receita real por imovel/dia; proximo passo e alimentar com observacoes reais do beta e avaliar import CSV/anfitriao. |
...
| F10 | P2 | Fluxo de upsell de quantity via Stripe Portal ou update direto | F5 | Backend cria sessao Stripe Billing Portal e `/my-plan` expoe CTA para assinaturas reais; falta validar portal/webhook em Stripe test mode. |
