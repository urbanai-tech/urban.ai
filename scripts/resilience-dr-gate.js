...
const contracts = [
  ...
  },
  {
    ...
    checks: [
      ...
      ['bucket policy mutation is explicit opt-in', /apply_backup_policy:[\s\S]*type:\s*boolean[\s\S]*default:\s*false/i],
      ['bucket mutation is restricted to canonical repository', /github\.repository\s*==\s*'urbanai-tech\/urban\.ai'\s*&&\s*inputs\.apply_backup_policy/i],
      ['versioning is configured explicitly', /put-bucket-versioning[\s\S]*Status=Enabled/i],
      ['lifecycle is configured explicitly', /put-bucket-lifecycle-configuration/i],
      ['current backups retain for 90 days', /Expiration:\s*\{Days:\s*90\}/i],
      ['noncurrent versions retain for 30 days', /NoncurrentVersionExpiration:\s*\{NoncurrentDays:\s*30\}/i],
      ['incomplete multipart uploads abort after 7 days', /AbortIncompleteMultipartUpload:\s*\{DaysAfterInitiation:\s*7\}/i],
      ['versioning fails closed', /Versionamento do bucket off-site nao esta habilitado[\s\S]*exit 1/i],
      ['canonical lifecycle rule fails closed', /Regra canonica de lifecycle nao encontrada[\s\S]*exit 1/i],
      ...
      ['restore evidence records current retention', /Current backup retention/i],
      ['restore evidence records noncurrent retention', /Noncurrent version retention/i],
      ['restore evidence records multipart cleanup', /Abort incomplete multipart uploads/i],
    ],
  },
  {
